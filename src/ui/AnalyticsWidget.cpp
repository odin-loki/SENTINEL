...
void AnalyticsWidget::buildCalibrationTab()
{
    ...
    // ── Header ───────────────────────────────────────────────────────────────
    auto* headerRow = new QHBoxLayout();
    auto* titleLbl  = new QLabel("Model Calibration Curve", container);
    titleLbl->setStyleSheet("color: #eaeaea; font-size: 15px; font-weight: bold;");

    auto* descLbl = new QLabel(
        "Reliability diagram: compares mean predicted probability vs. observed positive "
        "rate per decile. A perfectly calibrated model lies on the diagonal.",
        container);
    descLbl->setStyleSheet("color: #a0a8b8; font-size: 12px;");
    descLbl->setWordWrap(true);

    m_runCalibBtn = new QPushButton("Run Calibration Analysis", container);
    m_runCalibBtn->setMinimumWidth(200);
    m_runCalibBtn->setStyleSheet(
    ...
    headerRow->addWidget(titleLbl);
    headerRow->addStretch();
    headerRow->addWidget(m_runCalibBtn);
    layout->addLayout(headerRow);
    layout->addWidget(descLbl);

    // ── Summary label (ECE / MCE / ACE) ──────────────────────────────────────
    m_calibSummaryLbl = new QLabel("Run analysis to see calibration metrics.", container);
    m_calibSummaryLbl->setStyleSheet("color: #a0a8b8; font-size: 12px; padding: 4px 0;");
    m_calibSummaryLbl->setTextFormat(Qt::RichText);
    layout->addWidget(m_calibSummaryLbl);

    // ── Chart view ───────────────────────────────────────────────────────────
    m_calibView = new QChartView(container);
    m_calibView->setRenderHint(QPainter::Antialiasing);
    m_calibView->setMinimumHeight(320);

    // Placeholder empty chart
    auto* chart = new QChart();
    chart->setTitle("Reliability Diagram (run analysis to populate)");
    styleChart(chart);
    m_calibView->setChart(chart);

    layout->addWidget(m_calibView, 1);
    m_tabWidget->addTab(container, "Calibration");

    connect(m_runCalibBtn, &QPushButton::clicked, this, &AnalyticsWidget::runCalibration);
}

// ─────────────────────────────────────────────────────────────────────────────
void AnalyticsWidget::runCalibration()
{
    if (!m_calibView || !m_runCalibBtn) return;
    m_runCalibBtn->setEnabled(false);

    // ── Fetch events (same approach as runBenchmark) ──────────────────────────
    auto events = m_db->getAllEvents();
    std::sort(events.begin(), events.end(), [](const CrimeEvent& a, const CrimeEvent& b) {
        return a.occurredAt.value_or(a.ingestedAt) < b.occurredAt.value_or(b.ingestedAt);
    });
    ...
    if (events.size() < 20) {
        m_calibSummaryLbl->setText(
            "<span style='color:#ef5350;'>Calibration: insufficient data — need at least 20 events.</span>");
        m_runCalibBtn->setEnabled(true);
        return;
    }
    ...
    // ── Build (predicted, actual) pairs ───────────────────────────────────────
    QVector<QPair<double,double>> predActual;
    predActual.reserve((events.size() - trainSize) * 2);
    ...
    for (int i = trainSize; i < events.size(); ++i) {
        ...
        // Negative: same zone shifted −365 days
    ...
    chart->setTitle("Reliability Diagram — Predicted vs Observed Probability");
    styleChart(chart);

    auto* axisX = new QValueAxis();
    axisX->setTitleText("Mean Predicted Probability");
    ...
    auto* axisY = new QValueAxis();
    axisY->setTitleText("Fraction of Positives (Observed Rate)");
    ...
    // ── Summary metrics ───────────────────────────────────────────────────────
    const QString statusColour = (result.status() == "EXCELLENT") ? "#66bb6a"
                               : (result.status() == "GOOD")      ? "#4fc3f7"
                               : (result.status() == "FAIR")      ? "#ffa726"
                               :                                    "#ef5350";

    m_calibSummaryLbl->setText(
        QString("Calibration on %1 samples  |  "
                "ECE: <b>%2</b>  |  MCE: <b>%3</b>  |  ACE: <b>%4</b>  |  "
                "Brier: <b>%5</b>  |  "
                "Status: <b><span style='color:%6;'>%7</span></b>")
            .arg(result.nSamples)
